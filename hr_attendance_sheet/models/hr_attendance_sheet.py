...
class AttendanceSheet(models.Model):
    ...
    def _cron_generate_attendance_sheet(self):
        today = date.today()
        start_month = today + relativedelta(day=26)
        end_month = today + relativedelta(months=1, day=25)
    ...
    def check_contract(self):
        for att in self:
            if not att.contract_id:
                if (not att.employee_id) or (not att.date_from) or (not att.date_to):
                    continue
                employee = att.employee_id
                date_from = att.date_from
                date_to = att.date_to
                contracts = employee._get_contracts(date_from, date_to)
                att.contract_id = contracts[0]
    ...
    def _get_workday_lines(self):
        ...
        unpaid_leave_work_entry = work_entry_obj.search([('code', '=', 'ATTSHUL')])
        if not unpaid_leave_work_entry:
            raise ValidationError(_(
                'Please Add Work Entry Type For Attendance Sheet Unpaid Leave With Code ATTSHUL'))
        ...
        unpaid_leave = [{
            'name': "Unpaid Leave",
            'code': 'UPL',
            'work_entry_type_id': unpaid_leave_work_entry[0].id,
            'sequence': 66,
            'number_of_days': self.no_unpaid_leave,
            'number_of_hours': self.no_unpaid_leave,
        }]
        worked_days_lines = overtime + late + absence + difftime + unpaid_leave
    ...
    def create_payslip(self):
        ...
        for att_sheet in self:
            ...
            unpaid_leave = [{
                ...
                'contract_id': contract_id,
                'sequence': 66,
                'number_of_days': att_sheet.no_unpaid_leave,
                'number_of_hours': att_sheet.tot_unpaid_leave,
            }]

            worked_days_line_ids += overtime + late + absence + difftime + unpaid_leave
